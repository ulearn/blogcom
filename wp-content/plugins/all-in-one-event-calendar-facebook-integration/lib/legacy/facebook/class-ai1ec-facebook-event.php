			...
			try {
				$result = $this->save( $facebook, $data_to_send );
					...
					if( $e->getMessage() === self::EX_INVALID_TIME ) {
						...
						$message['message'] .= '<br />' . __( "If you are trying to create an event for a page, try refreshing the access Tokens. This is the error message: " , AI1EC_PLUGIN_NAME ) . $e->getMessage();
					} else {
						$message['message'] .= '<br />' . __( "This is the error message: " , AI1EC_PLUGIN_NAME ) . $e->getMessage();
					}
