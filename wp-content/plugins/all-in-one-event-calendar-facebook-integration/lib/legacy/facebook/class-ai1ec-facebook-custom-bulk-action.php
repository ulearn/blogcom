...
class Ai1ec_Facebook_Custom_Bulk_Action extends Ai1ec_Base {
	...
	public function __construct( Ai1ec_Registry_Object $registry ) {
		$this->facebook_plugin = $registry->get( 'calendar-feeds.facebook' );
	}
	/**
	...
	public function facebook_filter_posts_where( $where ) {
		...
		if( isset( $_GET[self::FB_SELECT_NAME] ) && ! empty( $_GET[self::FB_SELECT_NAME] ) ) {
			...
			global $wpdb;
			// update the query
			$where .= " AND ID in ( SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_ai1ec_facebook_status' AND meta_value = '$type' $end ) ";
		}
		return $where;
	}
	/**
		...
		if( ( ( isset( $_GET['action'] ) && $_GET['action'] === 'export-facebook' ) || ( isset( $_GET['action2'] ) && $_GET['action2'] === 'export-facebook' ) ) && isset( $_GET['post'] ) ) {
			...
			if( $this->facebook_plugin->check_if_we_have_a_valid_access_token() ) {
				...
				foreach( $_GET['post'] as $post_id ) {
					...
					$meta = $facebook_meta->get_facebook_meta( $post_id );
					if( empty( $meta['facebook_status'] ) ) {
						...
						if( is_array( $result )  && ! empty( $result ) ) {
							$facebook_meta = $this->_registry->get( 'facebook.meta' );
							$facebook_meta->save_facebook_eid( $post_id, $result['id'] );
							$facebook_meta->save_facebook_status( $post_id, Ai1ecFacebookConnectorPlugin::FB_EXPORTED_EVENT );
