 ...
 */
class Ai1ec_Time_Utility_Legacy
{
	...
	/**
	 * @var Ai1ec_Time_I18n_Utility_Legacy Instance of I18n time management utility
	 */
	...
	static public function instance() {
		if ( ! ( self::$_instance instanceof Ai1ec_Time_Utility_Legacy ) ) {
			self::$_instance = new Ai1ec_Time_Utility_Legacy();
		}
	...
	static public function get_local_timezone( $default = 'America/Los_Angeles' ) {
		...
		if ( NULL === $_cached ) {
			...
			$zone = '';
			if ( $user->ID > 0 ) {
				$zone = self::user_selected_tz( $user->ID );
			}
			unset( $user );
			if ( empty( $zone ) ) {
	 ...
	 */
	static public function user_selected_tz(
		$user_id,
		$new_value    = NULL,
		$force_update = false
	) {
	...
	protected function __construct() {
		...
		$this->_time_i18n        = new Ai1ec_Time_I18n_Utility_Legacy();
	}
